...
A Streamlit dashboard that polls `psutil` once per second, appends the reading to a `LiveFold`, and lets you query `{sum, max, min}` over any user-selected time window via a range slider.
...
The point of using `LiveFold` here — instead of just calling `sum/max/min(values[i:j])` — is that the per-query cost stays sublinear as the session grows. After an hour of polling, that's ~3,600 readings and a query touches ~60 of them per fold, not 3,600.
...
- Switch between CPU % and Memory % — the same `LiveFold` instance resets, but the API shape doesn't change.
